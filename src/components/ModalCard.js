...
export const ModalCard = ({ member, selectedMember, setSelectedMember }) => {
    const isSelected = selectedMember.includes(member.id) &&'border-success border-3'
    return (
        <div className={'border h-25 d-flex p-2 '+isSelected}  onClick={() => setSelectedMember([...selectedMember,member.id])}>
